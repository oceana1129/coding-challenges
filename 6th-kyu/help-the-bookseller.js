...
stocklist, with the category codes in the list of categories. Note: 
the codes are in the same order in both lists.

Return the result as a string described in the example below, or as 
a list of pairs (Haskell/Clojure/Racket/Prolog).

If any of the input lists is empty, return an empty string, or an 
empty array/list (Clojure/Racket/Prolog).
...
function stockList(books, categories) {
  ...
  categories.forEach((e) => map.set(e, 0));

  books.forEach((e) => {
    let category = e[0];
    let num = map.get(category);
    if (num !== undefined) {
      let quantity = Number(e.split(" ")[1]);
      map.set(category, num + quantity);
    }
  });
