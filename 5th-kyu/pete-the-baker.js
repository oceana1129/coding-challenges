...
for the amounts (e.g. 1 lb of flour or 200 g of sugar are simply 1 
or 200). Ingredients that are not present in the objects, can be 
considered as 0.
...
function cakes(recipe, available) {  
  ...
  for (let key in recipe) {
    let avail = available[key];
    if (typeof avail !== "number") return 0;
    
    avail = Math.floor(avail / recipe[key]);
    if (avail === 0) return 0;
    current.push(avail)
  }
